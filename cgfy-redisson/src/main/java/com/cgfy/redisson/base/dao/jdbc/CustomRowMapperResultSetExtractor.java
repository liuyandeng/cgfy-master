...
package com.cgfy.redisson.base.dao.jdbc;
 ...
 */
public class CustomRowMapperResultSetExtractor<T> implements ResultSetExtractor<List<T>>{

	private final RowMapper<T> rowMapper;

	private final int rowsExpected;
	
	private final int rowsQueryList;
	 ...
	 */
	public CustomRowMapperResultSetExtractor(RowMapper<T> rowMapper) {
		this(rowMapper, 0);
	}
	 ...
	 */
	public CustomRowMapperResultSetExtractor(RowMapper<T> rowMapper, int rowsExpected) {
		Assert.notNull(rowMapper, "RowMapper is required");
		this.rowMapper = rowMapper;
		this.rowsExpected = rowsExpected;
		this.rowsQueryList = 0;
	}
	 ...
	 * (just used for optimized collection handling)
	 */
	public CustomRowMapperResultSetExtractor(RowMapper<T> rowMapper, int rowsExpected, int rowsQueryList) {
		Assert.notNull(rowMapper, "RowMapper is required");
		this.rowMapper = rowMapper;
		this.rowsExpected = rowsExpected;
		this.rowsQueryList = rowsQueryList;
	}
	...
	public List<T> extractData(ResultSet rs) throws SQLException {
		...
		int rowNum = 0;
		int rowsQueryMax = this.rowsQueryList;
		while (rs.next()) {
			results.add(this.rowMapper.mapRow(rs, rowNum++));
			
			if (rowsQueryMax>0 && rowNum>rowsQueryMax)
			{
				throw new SQLException("结果集的大小为" + rowNum + "超过了设定最大值" + rowsQueryMax);
			}
		}
		return results;
	}
